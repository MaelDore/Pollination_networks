...
# Outputs:
   ...
   # Calibrated models for all possible combinations
...
# 1/ Generate summary table of all 3 web coverage x 6 response variables (VAE) = 18 models

# 2/ Model for all possible combinations

# Web coverage: full, dipt, hymeno                 # Coverage of the pollinator taxa included
# Model type: str, rich                            # For response variables of network strucutre, all networks with less than 5 plant or insects species OR less than 10 interactions are removed 
# Polar networks: no_polar                         # To decide whether or not to include the 3 extreme polar networks. In main text, polar networks are not included
# Response variables (VAE): C, Li, Lp, S, I, P     # 3 Structure variables + 3 Richness variables
...
all_models_summary <- data.frame(matrix(nrow = 36, ncol = 8, dimnames = list(NULL ,c("ID", "web_coverage", "model_type", "polar", "VAE", "model_dataset", "model_name", "Last_runtime"))))
...
polar_list <- "no_polar"                          # In main text, polar networks are not included
  ...
  for (model_type in model_type_list) {       # All model type : Structure or Richness
    ...
    for (polar in polar_list) {               # All choice for polar networks inclusion
      ...
      for (VAE in VAE_list) {    # All response variables
        ...
        all_models_summary$web_coverage[i] <- web_coverage
        all_models_summary$model_type[i] <- model_type
        all_models_summary$polar[i] <- polar
        all_models_summary$VAE[i] <- VAE
        all_models_summary$model_dataset[i] <- model_dataset
        all_models_summary$model_name[i] <- model_name
...
all_models_summary <- subset(all_models_summary, subset = !((all_models_summary$model_type == "rich") & (all_models_summary$VAE %in% c("C", "Li", "Lp"))))
all_models_summary <- subset(all_models_summary, subset = !((all_models_summary$model_type == "str") & (all_models_summary$VAE %in% c("S", "I", "P"))))

all_models_summary$ID <- 1:nrow(all_models_summary)      # Write model ID
row.names(all_models_summary) <- all_models_summary$ID   # Associate row.names with model ID

save(all_models_summary, file = "./Data/all_models_summary.RData")
...
k <- 1
for (j in 1:nrow(all_models_summary)) {

  # 2.1/ Define model specificity ####
  
  web_coverage <- all_models_summary$web_coverage[j] 
  model_type <- all_models_summary$model_type[j] 
  polar <- all_models_summary$polar[j] 
  VAE <- all_models_summary$VAE[j] 
  model_dataset <- all_models_summary$model_dataset[j]
  model_name <- all_models_summary$model_name[j]
  ...
  # View(models_table)
  save(models_table, file = paste0("./Models_outputs/",VAE,"/",web_coverage,"/",model_name,"/",model_name,"_Model_evaluation_table.RData")) # Save MAMs
  MAM_AICc <- list_potMAM_SEVM[which(models_table$delta_ME<=2)] # Extract MAMs after SEV correction (delta AICc < 2)
  saveRDS(MAM_AICc, file = paste0("./Models_outputs/",VAE,"/",web_coverage,"/",model_name,"/",model_name,"_MAM_AICc.RData")) # Save MAMs
  
  bestmod_AICc <- list_potMAM_SEVM[[which(models_table$delta_ME==0)]] # Extract best model after SEV correction
  saveRDS(bestmod_AICc, file = paste0("./Models_outputs/",VAE,"/",web_coverage,"/",model_name,"/",model_name,"_bestmod_AICc.RData")) # Save best model
  ...
  for (i in 1:length(VE_list)) {
    ...
    if (VE %in% names(models_table)) { # Check if this VE is present in models
      ...
      if (any(!is.na(eval(parse(text = paste0("models_table$",VE)))))) { # Check if at least one model use this VE
        ...
        saveRDS(eval(parse(text = paste0("bestmod_with_",VE_short_name))), file = paste0("./Models_outputs/",VAE,"/",web_coverage,"/",model_name,"/",model_name,"_bestmod_with_",VE_short_name,".RData"))
      } 
    }
  ...
  # load(file = paste0("./Models_outputs/Summaries/",model_name,"_summaries.RData"))
  # 
  ...
  # Clean environnement
  rm(list = setdiff(ls(), c("j", "k", "all_models_summary", "index_model_to_compute", "VE_list", "VE_short_name_list" , "aggreg.webs", "neigh.listw", "neighbour", "lim", "model_dataset", "model_name", "model_type", "model_type_list", "polar", "polar_list", "taxo", "taxo_list", "forest", "forest_list", "VAE", "VAE_list", "web_coverage", "web_coverage_list")))
  
  # Save last runtime
  
  all_models_summary$Last_runtime[j] <- as.character(Sys.time())
  
  # Affiche le temps de calcul et l'avancement
  cat(paste("\n", Sys.time(), "-", model_name, "-", j, "out of ", nrow(all_models_summary), " - Done \n"))
...
# Chose to include or not polar networks
# polar <- "with_polar"
polar <- "no_polar"
