...
# Inputs:
   ...
   # Results of calibrated models from Script SM6_1 (models, and summary tables)
   # Summary table of model parameters for model with sampling effects (all_models_summary)

# Outputs:

   # Summary array with all results (beta-coefficients, p-value, variable imoprtance) from model with and without sampling effects (Infos_all_models_sampling)
   # Summary table of model parameters for models with and without sampling effects (all_models_summary_compare)
   # Figure S6: Summary plots of beta-coefficients of best models including (red) or excluding (blue) sampling effects
...
# 3/ Loop to extract results into the array
   ...
   # Get beta-coefficients, standard error, p-value, and variable importance for each model

# 4/ Convert names of explanatory variables

# 5/ Plot all possible graphs, for all VAE

# 6/ Plot the final Figure

   # Plot each panel of the final plots indiviually, with custom layout corrections
...
sampling_list <- c("with_sampling", "no_sampling")
web_coverage_list <- c("full")
model_type_list <- c("str", "rich")
polar_list <- c("no_polar")
...
# Store results in array to be abel to use easily with ggplot later

Infos_all_models_sampling <- array(dim = c(length(sampling_list),      # With or without sampling variables
                                        length(VAE_list),           # Nb of VAE
                          ...
                          dimnames = list(sampling = sampling_list,          # With or without sampling variables
                                          VAE = VAE_list,                    # VAE names
                                          ...
                                          info = info_list)                  # Beta-coef of MAMs, sd, p-value, signifiance level, and importance of variable
)             

# save(Infos_all_models_sampling, file = "./Data/Infos_all_models_sampling.RData")

### 2/ Load summary tables ####

# Used to keep track of model parameters

# load(file = "./Data/Infos_all_models_sampling.RData")

# Merge summary table from results with sampling variables, and without

load(file = "./Data/all_models_summary.RData")
load(file = "./Data/all_models_summary_no_sampling.RData")

all_models_summary$sampling <- "with_sampling"
all_models_summary_no_sampling$sampling <- "no_sampling"

all_models_summary <- all_models_summary[all_models_summary$web_coverage == "full",]

all_models_summary_compare <- rbind(all_models_summary, all_models_summary_no_sampling)

# save(all_models_summary_compare, file = "./Data/all_models_summary_compare.RData")
...
for (j in 1:nrow(all_models_summary_compare)) {
  
  sampling <- all_models_summary_compare$sampling[j]
  web_coverage <- all_models_summary_compare$web_coverage[j] 
  model_type <- all_models_summary_compare$model_type[j] 
  polar <- all_models_summary_compare$polar[j] 
  VAE <- all_models_summary_compare$VAE[j]
  
  model_name <- all_models_summary_compare$model_name[j]
  
  if (sampling == "with_sampling") {
    ...
    # Load MAMs
    MAM_AICc <- readRDS(file = paste0("./Models_outputs_no_sampling/",VAE,"/",model_name,"/",model_name,"_MAM_AICc.RData")) 
    
  }
  
  for (VE in VE_list) {
    
    if (length(MAM_AICc) == 1) { # Case with only one MAM = best model, Summary has different format
      
      if (length(grep(VE, row.names(summary_MAM.avg.coefs_std))) != 0) { # Try to retrieve infos only if the VE is present in the MAM = best model
        
        # Retrieve beta-coef
        Infos_all_models_sampling[sampling, VAE, web_coverage, model_type, polar, VE, "Beta_coef"] <- 
          round(summary_MAM.avg.coefs_std                                      # Matrix of MAMs results
                [grep(VE, row.names(summary_MAM.avg.coefs_std)),1], 4)         # Extract beta-coef estimate                                      
        
        # Retrieve sd
        Infos_all_models_sampling[sampling, VAE, web_coverage, model_type, polar, VE, "SD"] <- 
          round(summary_MAM.avg.coefs_std                                      # Matrix of MAMs results
                [grep(VE, row.names(summary_MAM.avg.coefs_std)),2], 4)         # Extract sd 
        
        # Retrieve p-value
        Infos_all_models_sampling[sampling, VAE, web_coverage, model_type, polar, VE, "p-value"] <- 
          round(summary_MAM.avg.coefs_std                                      # # Matrix of MAMs results
                [grep(VE, row.names(summary_MAM.avg.coefs_std)),5], 4)         # Extract p-value 
        
        # Classify significance level regarding the p-value
        Infos_all_models_sampling[sampling, VAE, web_coverage, model_type, polar, VE, "Signif_lvl"] <- 0
        if (Infos_all_models_sampling[sampling, VAE, web_coverage, model_type, polar, VE, "p-value"] < 0.05) {
          Infos_all_models_sampling[sampling, VAE, web_coverage, model_type, polar, VE, "Signif_lvl"] <- 1
          
          if (Infos_all_models_sampling[sampling, VAE, web_coverage, model_type, polar, VE, "p-value"] < 0.01) {
            Infos_all_models_sampling[sampling, VAE, web_coverage, model_type, polar, VE, "Signif_lvl"] <- 2
            
            if (Infos_all_models_sampling[sampling, VAE, web_coverage, model_type, polar, VE, "p-value"] < 0.001) {
              Infos_all_models_sampling[sampling, VAE, web_coverage, model_type, polar, VE, "Signif_lvl"] <- 3
        ...
        Infos_all_models_sampling[sampling, VAE, web_coverage, model_type, polar, VE, "Importance"] <- 1 # All present variables have an importance of 1, because there is only one model
        
      }
      
      
    } else { # Case with several MAMs
      ...
      if (length(grep(VE, row.names(summary_MAM.avg.coefs_std$coefmat.full))) != 0) { # Try to retrieve infos only if the VE is present in one of the MAMs
        ...
        Infos_all_models_sampling[sampling, VAE, web_coverage, model_type, polar, VE, "Importance"] <- imp_var[grep(VE, names(imp_var))]
  ...
  # Check running
  cat(paste("\n Model n?", j, "on", nrow(all_models_summary_compare), "\n"))
...
VE_list <- c("ln_HF", "Tot_Rainfall_IPCC", "Mean_T_IPCC", "Rainfall_Seasonality_IPCC", "T_Seasonality_IPCC", "ln_sptot", "ln_pl", "ln_ins", "ln_SE", "ln_time", "ln_ATS", "Sampling_type", "taxo_full_sp_perc")
VE_short_name_list <-  c("HII", "Ptot", "Tmean", "Pvar", "Tvar", "S", "P", "I", "SE", "Time", "ATS", "Method", "Taxo")

Infos_all_models_sampling_VE_renamed <- Infos_all_models_sampling

dimnames(Infos_all_models_sampling_VE_renamed)$VE <- VE_short_name_list

# save(Infos_all_models_sampling_VE_renamed, file = "./Data/Infos_all_models_sampling_VE_renamed.RData")


### 5/ Loop to plot all possible graphs, for each VAE ####
...
for (VAE in VAE_list) {
  ...
  Infos_sub_models <- Infos_all_models_sampling[,VAE,,,,,] 
  ...
  pdf(file = paste0("./SM_Figures/SM_6_Summary_no_sampling/Beta_coeffs_summary_",VAE,".pdf"), width = 8, height = 6)
  ...
  g <- ggplot(Estimate_sub_models_df, aes(x = VE, y = Beta_coef, 
                                          ...
                                          # color = Estimate_sub_models_df$sampling, 
                                          # fill = Estimate_sub_models_df$sampling,
                                          color = as.factor(c("With", "Without"))[as.numeric(Estimate_sub_models_df$sampling)],
  ...
  g <- g + theme_bw() + ggtitle(paste("Models for", VAE_full_name_list[which(VAE_list == VAE)])) +  
    ...
    scale_y_continuous(limits = c(min(Estimate_sub_models_df$Beta_coef-SD_sub_models_df$SD), max(Estimate_sub_models_df$Beta_coef+SD_sub_models_df$SD)),
                       ...
                       breaks = seq(from = (floor(min(Estimate_sub_models_df$Beta_coef-SD_sub_models_df$SD)*10))/10, to = (ceiling(max(Estimate_sub_models_df$Beta_coef+SD_sub_models_df$SD)*10))/10, by = 0.2)) +
    guides(col=guide_legend("Sampling variables"),
           size=guide_legend("Significance levels")) +
    # Ligne ? y = 0
...
##### 6/ Final figure with all summary plot of each VAE in one figure #####
...
for (VAE in VAE) {
  ...
  g <- ggplot(Estimate_sub_models_df1, aes(x = VE, y = Beta_coef, 
                                           ...
                                           # color = Estimate_sub_models_df1$sampling, 
                                           # fill = Estimate_sub_models_df1$sampling,
                                           color = as.factor(c("With", "Without"))[as.numeric(Estimate_sub_models_df1$sampling)],
  ...
  g <- g + theme_bw() + ggtitle("(a) Species richness") +  
    ...
    scale_y_continuous(limits = c(min(Estimate_sub_models_df1$Beta_coef-SD_sub_models_df1$SD), max(Estimate_sub_models_df1$Beta_coef+SD_sub_models_df1$SD)),
                       # scale_y_continuous(limits = c(-0.15, 0.15),
                       breaks = seq(from = (floor(min(Estimate_sub_models_df1$Beta_coef-SD_sub_models_df1$SD)*10))/10, to = (ceiling(max(Estimate_sub_models_df1$Beta_coef+SD_sub_models_df1$SD)*10))/10, by = 0.2)) +
    guides(col=guide_legend("Sampling variables"),
           size=guide_legend("Significance levels")) +
    # Ligne ? y = 0
    geom_hline(yintercept = 0, linetype = "dashed", color = "black") 
  ...
  save(g1, Estimate_sub_models_df1, SD_sub_models_df1, Signif_sub_models_df1, file = paste0("./Data/SM_6/Compared_",VAE,"_plot_data.RData"))
...
for (VAE in VAE) {
  ...
  g <- ggplot(Estimate_sub_models_df2, aes(x = VE, y = Beta_coef, 
                                           ...
                                           # color = Estimate_sub_models_df2$sampling, 
                                           # fill = Estimate_sub_models_df2$sampling,
                                           color = as.factor(c("With", "Without"))[as.numeric(Estimate_sub_models_df2$sampling)],
  ...
  g <- g + theme_bw() + ggtitle("(b) Insect richness") +  
    ...
    scale_y_continuous(limits = c(min(Estimate_sub_models_df2$Beta_coef-SD_sub_models_df2$SD), max(Estimate_sub_models_df2$Beta_coef+SD_sub_models_df2$SD)),
                       # scale_y_continuous(limits = c(-0.15, 0.15),
                       breaks = seq(from = (floor(min(Estimate_sub_models_df2$Beta_coef-SD_sub_models_df2$SD)*10))/10, to = (ceiling(max(Estimate_sub_models_df2$Beta_coef+SD_sub_models_df2$SD)*10))/10, by = 0.2)) +
    guides(col=guide_legend("Sampling variables"),
           size=guide_legend("Significance levels")) +
    # Ligne ? y = 0
    geom_hline(yintercept = 0, linetype = "dashed", color = "black") 
  ...
  save(g2, Estimate_sub_models_df2, SD_sub_models_df2, Signif_sub_models_df2, file = paste0("./Data/SM_6/Compared_",VAE,"_plot_data.RData"))
...
for (VAE in VAE) {
  ...
  g <- ggplot(Estimate_sub_models_df3, aes(x = VE, y = Beta_coef, 
                                           ...
                                           # color = Estimate_sub_models_df3$sampling, 
                                           # fill = Estimate_sub_models_df3$sampling,
                                           color = as.factor(c("With", "Without"))[as.numeric(Estimate_sub_models_df3$sampling)],
  ...
  g <- g + theme_bw() + ggtitle("(c) Plant richness") +  
    ...
    scale_y_continuous(limits = c(min(Estimate_sub_models_df3$Beta_coef-SD_sub_models_df3$SD), max(Estimate_sub_models_df3$Beta_coef+SD_sub_models_df3$SD)),
                       # scale_y_continuous(limits = c(-0.15, 0.15),
                       breaks = seq(from = (floor(min(Estimate_sub_models_df3$Beta_coef-SD_sub_models_df3$SD)*10))/10, to = (ceiling(max(Estimate_sub_models_df3$Beta_coef+SD_sub_models_df3$SD)*10))/10, by = 0.2)) +
    guides(col=guide_legend("Sampling variables"),
           size=guide_legend("Significance levels")) +
    # Ligne ? y = 0
    geom_hline(yintercept = 0, linetype = "dashed", color = "black") 
  ...
  save(g3, Estimate_sub_models_df3, SD_sub_models_df3, Signif_sub_models_df3, file = paste0("./Data/SM_6/Compared_",VAE,"_plot_data.RData"))
...
for (VAE in VAE) {
  ...
  g <- ggplot(Estimate_sub_models_df4, aes(x = VE, y = Beta_coef, 
                                           ...
                                           # color = Estimate_sub_models_df4$sampling, 
                                           # fill = Estimate_sub_models_df4$sampling,
                                           color = as.factor(c("With", "Without"))[as.numeric(Estimate_sub_models_df4$sampling)],
  ...
  g <- g + theme_bw() + ggtitle("(d) Connectance") +  
    ...
    scale_y_continuous(limits = c(min(Estimate_sub_models_df4$Beta_coef-SD_sub_models_df4$SD), max(Estimate_sub_models_df4$Beta_coef+SD_sub_models_df4$SD)),
                       # scale_y_continuous(limits = c(-0.15, 0.15),
                       breaks = seq(from = (floor(min(Estimate_sub_models_df4$Beta_coef-SD_sub_models_df4$SD)*10))/10, to = (ceiling(max(Estimate_sub_models_df4$Beta_coef+SD_sub_models_df4$SD)*10))/10, by = 0.2)) +
    guides(col=guide_legend("Sampling variables"),
           size=guide_legend("Significance levels")) +
    # Ligne ? y = 0
    geom_hline(yintercept = 0, linetype = "dashed", color = "black") 
  ...
  save(g4, Estimate_sub_models_df4, SD_sub_models_df4, Signif_sub_models_df4, file = paste0("./Data/SM_6/Compared_",VAE,"_plot_data.RData"))
...
for (VAE in VAE) {
  ...
  g <- ggplot(Estimate_sub_models_df5, aes(x = VE, y = Beta_coef, 
                                           ...
                                           # color = Estimate_sub_models_df5$sampling, 
                                           # fill = Estimate_sub_models_df5$sampling,
                                           color = as.factor(c("With", "Without"))[as.numeric(Estimate_sub_models_df5$sampling)],
  ...
  g <- g + theme_bw() + ggtitle("(e) Insect link density") +  
    ...
    scale_y_continuous(limits = c(min(Estimate_sub_models_df5$Beta_coef-SD_sub_models_df5$SD), max(Estimate_sub_models_df5$Beta_coef+SD_sub_models_df5$SD)),
                       # scale_y_continuous(limits = c(-0.15, 0.15),
                       breaks = seq(from = (floor(min(Estimate_sub_models_df5$Beta_coef-SD_sub_models_df5$SD)*10))/10, to = (ceiling(max(Estimate_sub_models_df5$Beta_coef+SD_sub_models_df5$SD)*10))/10, by = 0.1)) +
    guides(col=guide_legend("Sampling variables"),
           size=guide_legend("Significance levels")) +
    # Ligne ? y = 0
    geom_hline(yintercept = 0, linetype = "dashed", color = "black") 
  ...
  save(g5, Estimate_sub_models_df5, SD_sub_models_df5, Signif_sub_models_df5, file = paste0("./Data/SM_6/Compared_",VAE,"_plot_data.RData"))
...
for (VAE in VAE) {
  ...
  g <- ggplot(Estimate_sub_models_df6, aes(x = VE, y = Beta_coef, 
                                           ...
                                           # color = Estimate_sub_models_df6$sampling, 
                                           # fill = Estimate_sub_models_df6$sampling,
                                           color = as.factor(c("With", "Without"))[as.numeric(Estimate_sub_models_df6$sampling)],
  ...
  g <- g + theme_bw() + ggtitle("(f) Plant link density") +  
    ...
    scale_y_continuous(limits = c(min(Estimate_sub_models_df6$Beta_coef-SD_sub_models_df6$SD), max(Estimate_sub_models_df6$Beta_coef+SD_sub_models_df6$SD)),
                       # scale_y_continuous(limits = c(-0.15, 0.15),
                       breaks = seq(from = (floor(min(Estimate_sub_models_df6$Beta_coef-SD_sub_models_df6$SD)*10))/10, to = (ceiling(max(Estimate_sub_models_df6$Beta_coef+SD_sub_models_df6$SD)*10))/10, by = 0.2)) +
    guides(col=guide_legend("Sampling variables"),
           size=guide_legend("Significance levels")) +
    # Ligne ? y = 0
    geom_hline(yintercept = 0, linetype = "dashed", color = "black") 
  ...
  save(g6, Estimate_sub_models_df6, SD_sub_models_df6, Signif_sub_models_df6, file = paste0("./Data/SM_6/Compared_",VAE,"_plot_data.RData"))
...
load(file = "./Data/SM_6/Compared_C_plot_data.RData")
load(file = "./Data/SM_6/Compared_Li_plot_data.RData")
load(file = "./Data/SM_6/Compared_Lp_plot_data.RData")
load(file = "./Data/SM_6/Compared_S_plot_data.RData")
load(file = "./Data/SM_6/Compared_I_plot_data.RData")
load(file = "./Data/SM_6/Compared_P_plot_data.RData")
...
pdf(file = paste0("./SM_Final_figures/Figure_S6.pdf"), width = 15, height = 8)
