...
for (j in 1:nrow(all_models_summary_no_polar)) {
  ...
  if (!dir.exists(paths = paste0("./Models_outputs_with_polar/",VAE,"/",model_name))) { # Revoie T/F si le dossier existe
    dir.create(path = paste0("./Models_outputs_with_polar/",VAE,"/",model_name), recursive = T) # Créer un (sous-)dossier. Récursive = T => pour aussi générer les dossiers parents si inexistents
  } 
