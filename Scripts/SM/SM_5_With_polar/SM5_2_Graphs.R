...
##### Script SM5_2: Generate Figure S5 #####
...
# Outputs:
    ...
    # Summary array with all results (beta-coefficients, p-value, variable imoprtance) from model with and without polar networks (Infos_all_models_polar)
    # Summary table of model parameters for models with and without polar netwroks (all_models_summary_polar)
    # Figure S5: Summary plots of beta-coefficients of best models including (blue) or excluding (red) polar networks
  ...
  for (VE in VE_list) {
    ...
    if (length(MAM_AICc) == 1) { # Case with only one MAM = best model, Summary has different format
      ...
      if (length(grep(VE, row.names(summary_MAM.avg.coefs_std))) != 0) { # Try to retrieve infos only if the VE is present in the MAM = best model
        ...
        # Retrieve beta-coef
        Infos_all_models_polar[VAE, web_coverage, model_type, polar, VE, "Beta_coef"] <- 
        ...
        # Retrieve sd
        Infos_all_models_polar[VAE, web_coverage, model_type, polar, VE, "SD"] <- 
          round(summary_MAM.avg.coefs_std                                      # Matrix of MAMs results
                [grep(VE, row.names(summary_MAM.avg.coefs_std)),2], 4)         # Extract sd 
        
        # Retrieve p-value
        Infos_all_models_polar[VAE, web_coverage, model_type, polar, VE, "p-value"] <- 
          round(summary_MAM.avg.coefs_std                                      # Matrix of MAMs results
                [grep(VE, row.names(summary_MAM.avg.coefs_std)),5], 4)         # Extract p-value 
        
        # Classify significance level regarding the p-value
        Infos_all_models_polar[VAE, web_coverage, model_type, polar, VE, "Signif_lvl"] <- 0
        if (Infos_all_models_polar[VAE, web_coverage, model_type, polar, VE, "p-value"] < 0.05) {
          Infos_all_models_polar[VAE, web_coverage, model_type, polar, VE, "Signif_lvl"] <- 1
          
          if (Infos_all_models_polar[VAE, web_coverage, model_type, polar, VE, "p-value"] < 0.01) {
            Infos_all_models_polar[VAE, web_coverage, model_type, polar, VE, "Signif_lvl"] <- 2
            
            if (Infos_all_models_polar[VAE, web_coverage, model_type, polar, VE, "p-value"] < 0.001) {
              Infos_all_models_polar[VAE, web_coverage, model_type, polar, VE, "Signif_lvl"] <- 3
        ...
        Infos_all_models_polar[VAE, web_coverage, model_type, polar, VE, "Importance"] <- 1 # All present variables have an importance of 1, because there is only one model
      ...
      if (length(grep(VE, row.names(summary_MAM.avg.coefs_std$coefmat.full))) != 0) { # Try to retrieve infos only if the VE is present in one of the MAMs
        ...
        Infos_all_models_polar[VAE, web_coverage, model_type, polar, VE, "Importance"] <- imp_var[grep(VE, names(imp_var))]
...
##### 5/ Loop to plot graphs for each VAE ####
...
for (VAE in VAE_list) {
  ...
  Infos_sub_models <- Infos_all_models_polar[VAE,,,,,] 
...
##### 6/ Final figure with all summary plot for each VAE in one figure ####

# Need to generate each plot individually with its specificity, then dorganize them all together on one figure
...
pdf(file = paste0("./SM_Final_figures/Figure_S5.pdf"), width = 15, height = 8)
